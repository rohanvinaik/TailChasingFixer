...
def load_analyzers(config: Dict[str, Any]) -> List[Analyzer]:
    ...
    # Add LSH duplicate analyzer if enabled (most efficient) - DEFAULT ON
    lsh_config = config.get("lsh_duplicates", {})
    if lsh_config.get("enabled", True) and LSH_DUPLICATE_AVAILABLE:  # Default to True
    ...
    elif config.get("semantic_accelerated", {}).get("enabled", False) and SEMANTIC_ACCELERATED_AVAILABLE:
